...
const TOPIC_SCHEMA_EXPECTED_SECTION_HEADINGS = [
  "Summary",
...
const TOPIC_SCHEMA_VALIDATION_GROUP_HEADINGS = [
  "Topic Scope",
  "Topic Body",
...
const TOPIC_SCHEMA_VALIDATION_CATEGORY_LABELS = [
  "Allowed Shapes",
  "Applies To",
  "Optional Fields",
  "Optional Sections",
...
const TOPIC_SCHEMA_ARTIFACT_CREATION_GROUP_HEADINGS = [
  "Prompt Fields",
  "Template Body"
];

const TOPIC_SCHEMA_ARTIFACT_CREATION_CATEGORY_LABELS = [
  "Optional Fields",
  "Required Fields",
  "Required Shape",
  "Rules"
...
function addRedeclaredInheritedContractCategoryFindings(findings, filePath, declaredCategoryLabels, inheritedCategoryLabels, overriddenCategoryLabels) {
  ...
  findings.push({
    code: "topic-schema-contract-extension-redeclares-inherited-category-label",
...
function validateTraceableTopicSchemaSync(input) {
  ...
  if (parentSchemaPath && !parsed.parentCreatedAt) {
    findings.push({
      code: "topic-schema-parent-created-at-missing",
      category: "schema-note-lineage",
      filePath: input.filePath,
      message: "The topic schema Parent Created At is required when a parent trace is declared.",
      severity: "error"
    });
  } else if (parsed.parentCreatedAt && !isTraceableContinuityTimestamp(parsed.parentCreatedAt)) {
    findings.push({
      code: "topic-schema-parent-created-at-invalid",
      category: "schema-note-lineage",
      filePath: input.filePath,
      message: "The topic schema Parent Created At is not in the expected YYYY-MM-DD hh:mm:ss shape.",
      severity: "error"
    });
  } else if (parsed.parentCreatedAt && expectedParentCreatedAt && parsed.parentCreatedAt !== expectedParentCreatedAt) {
    findings.push({
      code: "topic-schema-parent-created-at-mismatch",
      category: "schema-note-lineage",
      filePath: input.filePath,
      message: `The topic schema Parent Created At must match the parent artifact Current Created At: ${expectedParentCreatedAt}.`,
      severity: "error"
    });
  }
  ...
  if ((parsed.currentSchema?.label ?? parsed.currentSchema?.target) !== "tiinex.topic.v1") {
    findings.push({
      ...
      category: "schema-note-lineage",
      filePath: input.filePath,
      message: "The topic validator expects Current Schema to be tiinex.topic.v1.",
      severity: "error"
    });
  }
  ...
  if ((parsed.envelopeSchema?.label ?? parsed.envelopeSchema?.target) !== "tiinex.root.v1") {
    findings.push({
      code: "topic-schema-envelope-schema-mismatch",
      category: "schema-note-lineage",
      filePath: input.filePath,
      message: "The topic schema must declare tiinex.root.v1 as its Envelope Schema.",
      severity: "error"
    });
  }
  ...
  if (!canReadResolvedSchemaPath(envelopeSchemaPath, input.readTextFileSync)) {
    findings.push({
      code: "topic-schema-envelope-schema-unreadable",
      category: "schema-note-lineage",
      filePath: input.filePath,
      message: "The topic schema Envelope Schema target could not be read.",
      severity: "error"
    });
  }
  ...
  if (!canReadResolvedSchemaPath(currentSchemaPath, input.readTextFileSync)) {
    findings.push({
      code: "topic-schema-current-schema-unreadable",
      category: "schema-note-lineage",
      filePath: input.filePath,
      message: "The topic schema Current Schema target could not be read.",
      severity: "error"
    });
  }
  ...
  if ((parsed.parentSchema?.label ?? parsed.parentSchema?.target) !== "tiinex.root.v1") {
    findings.push({
      code: "topic-schema-parent-schema-mismatch",
      category: "schema-note-lineage",
      filePath: input.filePath,
      message: "The topic schema must declare tiinex.root.v1 as its direct parent schema.",
      severity: "error"
    });
  }
  ...
  if (!canReadResolvedSchemaPath(declaredParentSchemaPath, input.readTextFileSync)) {
    findings.push({
      code: "topic-schema-parent-schema-unreadable",
      category: "schema-note-lineage",
      filePath: input.filePath,
      message: "The topic schema Parent Schema target could not be read.",
      severity: "error"
    });
  }
  ...
  if (parsed.parentSchema?.label ?? parsed.parentSchema?.target ?? parsed.parentTrace?.label ?? parsed.parentTrace?.target) {
    if (!parsed.parentOrigin) {
      findings.push({
        code: "topic-schema-parent-origin-missing",
        category: "schema-note-lineage",
        filePath: input.filePath,
        message: "The topic schema must declare Parent Origin when it declares a parent.",
        severity: "error"
      });
    } else if (!parsed.parentOrigin.browseGit) {
      findings.push({
        code: "topic-schema-parent-origin-browse-git-missing",
        category: "schema-note-lineage",
        filePath: input.filePath,
        message: "The topic schema must include a commit-pinned Parent Origin browse + git permalink.",
        severity: "error"
      });
    } else if (!isCommitPinnedBrowseGitTarget(parsed.parentOrigin.browseGit)) {
      findings.push({
        code: "topic-schema-parent-origin-unpinned-browse-git",
        category: "schema-note-lineage",
        filePath: input.filePath,
  ...
  if ((parsed.footerIntegrity?.towardsLabel ?? parsed.footerIntegrity?.towardsTarget) !== "self"
    ...
    findings.push({
      ...
      category: "schema-note-lineage",
      filePath: input.filePath,
      message: "The topic schema footer should target the root schema or self according to the active validation strategy.",
      severity: "error"
    });
  }
  ...
  if (parsed.footerIntegrity?.towardsTarget === "self") {
    ...
    findings.push({
      ...
      category: "schema-note-lineage",
      filePath: input.filePath,
      message: "The topic schema footer Towards target must match the Parent Origin browse + git permalink.",
      severity: "error"
    });
  }

  collectExpectedSchemaHeadingFindings(findings, input.filePath, parsed, {
    codePrefix: "topic-schema-layout",
    displayName: "topic schema",
    titleHeading: "Topic",
    expectedSectionHeadings: TOPIC_SCHEMA_EXPECTED_SECTION_HEADINGS
  });

  if (!parsed.schemaValidationContract?.present) {
    findings.push({
      code: "topic-schema-validation-contract-missing",
      category: "schema-validation-contract",
      filePath: input.filePath,
      message: "The topic schema must include a Schema Validation Contract section.",
      severity: "error"
    });
  } else {
    addContractSectionShapeFindings(findings, input.filePath, parsed.schemaValidationContract, {
      codePrefix: "topic-schema-contract",
      category: "schema-validation-contract",
      displayName: "topic schema validation contract"
    });
    collectContractVocabularyFindings(findings, input.filePath, parsed.schemaValidationContract, {
      codePrefix: "topic-schema-contract",
      category: "schema-validation-contract",
      displayName: "topic schema validation contract",
      allowedGroupHeadings: [...TOPIC_SCHEMA_VALIDATION_GROUP_HEADINGS, "Contract Category Extension", "Contract Category Override"],
      allowedCategoryLabels: [...TOPIC_SCHEMA_VALIDATION_CATEGORY_LABELS, ...declaredContractCategoryLabels],
    ...
    addMissingGroupFindings(findings, input.filePath, parsed.schemaValidationContract, {
      ...
      category: "schema-validation-contract",
      displayName: "topic schema validation contract",
      requiredGroupHeadings: TOPIC_SCHEMA_VALIDATION_GROUP_HEADINGS
    });
  }

  if (!parsed.artifactCreationContract?.present) {
    findings.push({
      code: "topic-schema-artifact-creation-contract-missing",
      category: "artifact-creation-contract",
      filePath: input.filePath,
      message: "The topic schema must include an Artifact Creation Contract section.",
      severity: "error"
    });
  } else {
    addContractSectionShapeFindings(findings, input.filePath, parsed.artifactCreationContract, {
      codePrefix: "topic-schema-artifact-creation-contract",
      category: "artifact-creation-contract",
      displayName: "topic schema artifact creation contract"
    });
    collectContractVocabularyFindings(findings, input.filePath, parsed.artifactCreationContract, {
      codePrefix: "topic-schema-artifact-creation-contract",
      category: "artifact-creation-contract",
      displayName: "topic schema artifact creation contract",
      allowedGroupHeadings: TOPIC_SCHEMA_ARTIFACT_CREATION_GROUP_HEADINGS,
      allowedCategoryLabels: TOPIC_SCHEMA_ARTIFACT_CREATION_CATEGORY_LABELS,
      unexpectedSeverity: unknownContractSeverity
    });
    addMissingGroupFindings(findings, input.filePath, parsed.artifactCreationContract, {
      ...
      category: "artifact-creation-contract",
      displayName: "topic schema artifact creation contract",
      requiredGroupHeadings: TOPIC_SCHEMA_ARTIFACT_CREATION_GROUP_HEADINGS
    });
  }

  if (!parentSchemaPath) {
    findings.push({
      code: "topic-schema-parent-trace-unresolvable",
      category: "schema-note-lineage",
      filePath: input.filePath,
      message: "The topic schema must point to a resolvable root schema trace.",
      severity: "error"
    });
  } else {
    ...
    if (rootBlockingFindings.length > 0) {
      findings.push({
        code: "topic-schema-parent-root-invalid",
        category: "schema-note-lineage",
        filePath: input.filePath,
        message: "The topic schema points to a parent root schema that does not satisfy the root validator.",
        severity: "error"
      });
    }
  ...
  collectUnexpectedContinuityEnvelopeFieldFindings(findings, input.filePath, parsed, {
    codePrefix: "topic-schema-lineage",
    category: "schema-note-lineage",
    displayName: "topic schema continuity envelope",
