...
async function main() {
  ...
  assert.ok(packageJson.contributes?.configuration?.properties?.["tiinex.aiProvenance.defaultView"], "TRACEABLE default view setting is missing.");
  ...
  assert.ok(packageJson.contributes?.configuration?.properties?.["tiinex.aiProvenance.allRolesAvailableAsChatSender"], "TRACEABLE chat sender-role setting is missing.");
  assert.ok(packageJson.contributes?.configuration?.properties?.["tiinex.aiProvenance.defaultChatSenderRole"], "TRACEABLE default chat sender-role setting is missing.");
  ...
  assert.ok(panelSource.includes('label: "Task input"'), "Traceable panel source should render task input as its own chat row.");
  assert.ok(panelSource.includes('label: "User input"'), "Traceable panel source should label the primary prompt row as User input.");
  ...
  assert.ok(panelSource.includes('label: "User input",') && panelSource.includes('occurredAt: snapshot.startedAt,') && !panelSource.includes('label: "User input",\n      text: userInput,\n      role: "input",\n      occurredAt: snapshot.startedAt,\n      updatedAt: snapshot.updatedAt,'), "Traceable panel source should timestamp settled User input rows from their own occurredAt rather than the final output updatedAt.");
  ...
  assert.ok(panelSource.includes("chatViewEnabled: coerceBooleanLike(state.chatViewEnabled, false)") && panelSource.includes("followLatest: coerceBooleanLike(state.followLatest, true)"), "Traceable panel source should read restored chat booleans through the boolean-like coercion helper.");
  ...
  assert.ok(extensionSource.includes("defaultChatSenderRole,") && extensionSource.includes("async () => resolveConfiguredDefaultChatSenderRole()"), "Extension source should pass the configured default sender role into the TRACEABLE chat UI only.");
